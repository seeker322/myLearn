(function(a) {
    ...
    a.InitPage = function(w, $) {
        return {
			http:"http://exam.tiandaoedu.com/index.php/App/",
            init: function() {
	...
	a.CreateQuestions = function(w, $){
        return {
						...
						$("section:first").find(".swiper-pagination-hh span").each(function(){
							...
							if(_this.hasClass("cur")){
								var index = _this.index();
									iscollection = curSection.find(".questions").eq(index).attr("iscollection");
							}
						});
					}else{
						iscollection = $("section:first").attr("iscollection");
					}
					if($("section:first").attr("iscollection") == 1){
						$(".xh_xx").addClass("on");
						window.flag= true;
					}else{
             ...
             * @return {[type]} [description]
             */
             bindUpDownEvent: function () {
                // TODO:中间的条,动态改变.read_ques .pd30的max-height
                // http://bbs.csdn.net/topics/390429202
                // http://www.zhangxinxu.com/wordpress/2010/03/javascript实现最简单的拖拽效果/
                var listenJQ = $('.xh_hui'),
                ...
                listenJQ.on('touchend', function (event) {
            ...
            getData: function (callback, errorCallback) {
                var datas;
                if(w._debug){
                    $.get(InitPage.http+'Problem/detail', function (data) {
                        if(data.status == '1'){
            ...
            getAnswersHtml: function (answers) {
				...
				for(var key in answers){
					dl += '<dl><dt>' + key + '</dt><dd>' + answers[key] + '</dd></dl>';
				}
                return dl;
            },
            getChooseQuestionView: function (chooseData) {
                var answersHtml = this.getAnswersHtml(chooseData.smalls[0].answer);
                var html =  '<section class="xhQues section2 swiper-slide" iscollection="' + chooseData.smalls[0].is_collection + '" data-id="' + chooseData.big_id + '" sort="' + chooseData.big_sort + '">\
            				<div class="choice_ques">\
							<div class="queBoxs" data-smallId="' + chooseData.smalls[0].small_id + '">\
                            ' + chooseData.smalls[0].stem + '\
			...
			getReadAtricle:function(readData){
				...
				$.each(readData, function(i, obj){
					p += '<p data-sort="' + obj.sort + '">' + obj.content + '</p>';
				});
				return p;
			},
			getQuestionHtml:function(questionsArr){
				...
				$.each(questionsArr, function(i, obj){
					...
					question += '<div class="questions swiper-slide" sort="' + obj.sort_correct + '" iscollection="' + obj.is_collection + '"><h4>问题' +arr1[i]+ '</h4><div class="mb30 quesTit" data-smallId="' + obj.small_id + '">' + obj.stem + '</div><div class="xh_choise">' + questionChoice + '</div></div>';
				});
				return question;
			},
    ...
    a.Start = function(w, $) {
        return {
			...
			getChoiceAnswers:function(style, _this, userId, paperId, answer, smallId, time, isA){
				...
				var https = window.location.hash,  //含有#从套题诊断的练习进来的
					isAnums = https.indexOf("#");
				if(isA == "a" || (isAnums != (-1))){  //第一次练习进来
					$.get(InitPage.http+'UserSmall/answer',{"userId":userId,"paperId":paperId,"answer":answer,"smallId":smallId,"time":time}, function (data) {
						if(data.status == "1"){
							if(data.value.is_correct == "1"){ //答对
								if(style == 2){ //read
									...
									if(_this.parents(".questions").index() == _this.parents("section").find(".questions").length - 1 && $(".curItem i").text() == $(".count").text()){
										var next ='<p class="next"><a href="javascript:;" class="checkReport">查看套题报告</a></p>';
										_this.parents("section").append(next);
										t.CheckReport();
									}
								}else{
							...
							if(_this.parents(".questions").index() != _this.parents("section").find(".questions").length-1){
								...
								if($(".curItem i").text() == $(".count").text()){
									if(JSON.parse(localStorage.setting).isShowAnalyze == 0 && style == 2){
										var next ='<p class="next"><a href="javascript:;" class="checkReport">查看套题报告</a></p>';
											_this.parents("section").append(next);
											t.CheckReport();
									}else{
										_this.parents("section").find(".next").show().find("a").html("查看套题报告").addClass("checkReport");
										t.CheckReport();
									}
									
								}else{
									_this.parents("section").find(".next a").html("下一题");
								}
							}
							_this.parents("section").removeClass("swiper-no-swiping");
						}     
					...
					$.get(InitPage.http+'UserSmall/answer',{"userId":userId,"answer":answer,"smallId":smallId,"time":time}, function (data) {
						 if(data.status == "1"){
							if(data.value.is_correct == "1"){ //答对
								if(style == 2){ //read
									t.slideQuestion(_this);
			...
			nextQuestions:function(_this, style){
				...
				_this.parents("section").find(".next").off().on("click", function() {
                    var curIndex,isLast,
					curSort = _this.parents("section").attr("sort");
					if(style == 2){ //read
			...
			theLast:function(_this){
				...
				//}else{
					if($(".curItem i").text() == $(".count").text() && unfloadjx == 0){ //不展开解析并且是最后一题
						var next ='<p class="next"><a href="javascript:;" class="checkReport">查看套题报告</a></p>';
						_this.parents("section").append(next);
					}else{
						_this.parents("section").find(".next").show().find("a").html("查看套题报告").addClass("checkReport");
			...
			Slide:function(_this, num, style){
				...
				if(num == 1){ //答对时
					if(curIndex == isLast){ //最后一题
						$(".list1 span").off("click");
						$(".curItem i").html(curSort);
						if(unfloadjx == 0 && $(".curItem i").text() == $(".count").text()){
							var next ='<p class="next"><a href="javascript:;" class="checkReport">查看套题报告</a></p>';
							_this.parents("section").append(next);
						}else{
							if(style == 2 && $(".curItem i").text() != $(".count").text()){
								//
							}else{
								var next ='<p class="next"><a href="javascript:;" class="checkReport">查看套题报告</a></p>';
								_this.parents("section").append(next);	
					...
					if(curIndex == isLast){ //最后一题
						if($(".curItem i").text() != $(".count").text()){
							_this.parents(".questions").find(".next a").html("下一题");
						}else{
							_this.parents("section").find(".next").show().find("a").html("查看套题报告").addClass("checkReport");
						}
						$(".curItem i").html(curSort);
					...
					//	_this.parents("section").find(".next").html('<a href="javascript:;">下一题</a>');
						$(".curItem i").html(curSort);
					}
					$(".list1 span").off("click");
					t.isFormLianxi(_this);
			...
			CheckReport:function(){
				$(document).on("click", ".checkReport", function(){
					...
					userId = JSON.parse(sessionStorage.obj).value.user_id;
					 $.get(InitPage.http+"UserSmall/analysis",{"user_id":userId,"paper_id":paperId},function(data){
						if(data.status == "1"){
			   ...
			   var jxData = JSON.parse(sessionStorage.obj1).value.result;
				...
				$.each(jxData, function(i, obj){
					...
					if(_that.eq(i-1).attr("sort") == $(".count").text()){
						_that.eq(i-1).find(".next").show().find("a").html("查看套题报告").addClass("checkReport");
						Start.CheckReport();
					}
					/* 对应答案 */
    ...
    a.Collect = function(w, $) {
        return {
			...
			collectQuestion:function(){
				...
				if (!window.flag) { //收藏
					$.get(InitPage.http+"UserSmall/collection",{"userId":userId,"smallId":smallId},function(data){
						if(data.status == "1"){
					...
					$.get(InitPage.http+"UserSmall/cancel",{"userId":userId,"smallId":smallId},function(data){
						if(data.status  == "1"){
			...
			isCollect:function(){
				...
				$("section").each(function(){
					var _that = $(this);
					if(_that.attr("types") == 2){
						CreateQuestions.initEvent();  //拖动
						_that.find(".swiper-pagination-hh span:first").addClass("cur").siblings().removeClass("cur");
						_that.find(".quesBoxss").height($(this).find(".questions:first").outerHeight(true));
					}/* else{
						if($(this).attr("iscollection") == 1 && ($(this).hasClass("swiper-slide-active"))){ //已收藏并且是当前
							$(".xh_xx").addClass("on");
							window.flag = true;
						}else{
							$(".xh_xx").removeClass("on");
    ...
    a.MakeNotes = function(w, $) {
        return {
                ...
                $(".xh_note").on("click", function() {
					$("section").each(function(){
						if($(this).hasClass("swiper-slide-active")){
							if($(this).attr("types") == "2"){
								smallId	= $(this).find(".quesTit").data("smallid");
							}else{
			...
			noteSelect:function(userId){
				$.get(InitPage.http + "Book/listFolder",{"userId":userId},function(data){
					if(data.status == "1"){
						var noteLi=" ";
			...
			createNote:function(userId, smallId){
				$.get(InitPage.http+"Book/newBook",{"userId":userId,"smallId":smallId},function(data){
					if(data.status == "1"){
			...
			saveNote:function(userId, smallId, userText, folderId){
				$.get(InitPage.http+"Book/saveBook",{"userId":userId,"userText":userText,"smallId":smallId,"folderId":folderId},function(data){
					//	console.log(data);
				});
			}
        }
    ...
    a.Setting = function(w, $) {
        return {
            ...
            saveSetting: function (setting) {
				...
				var userId = JSON.parse(sessionStorage.obj).value.user_id;
                $.get(InitPage.http+"/User/saveSetting",
					{
						"userId":userId,
